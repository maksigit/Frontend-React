...
import './item-quarters-residential.scss';

class ItemQuartersResidential extends Component {
    ...
    defaultStyle = {
        ...
        backgroundImage: replace_path(`url(${STATIC_FILE}${this.props.item.photoMain[0].path})`)
    };
    ...
    animation = ({ target }) => {
        const { language, link, singleFetchData } = this.props;
        singleFetchData({ id: link, language: translate_language(language) });
        ...
        this.div.current.addEventListener('animationend', () => {
            this.props.history.push(`/${language}/living_complex/${link}`);
    ...
    render() {
        ...
        const { link, linkmap, img, title, location, deadline, status, flexdir = false, revers = false, fullWith = false, language, currentCategory } = this.props;
        return (
            <>
                ...
                <div className={classNames('wrap-item-residential', {
                    ...
                    <div className="item-residential__container">
                        <div className={classNames('item-residential')}>
...
export default ItemQuartersResidential;
